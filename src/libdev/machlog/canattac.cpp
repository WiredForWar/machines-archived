...
{
    bool result = candidateActor.objectIsCanAttack() and !(candidateActor.objectType() == MachLog::POD)
        && !(candidateActor.objectIsMissileEmplacement()
             && (candidateActor.asMissileEmplacement().subType() == MachPhys::ICBM
                 || !candidateActor.asMissileEmplacement().isComplete()));
